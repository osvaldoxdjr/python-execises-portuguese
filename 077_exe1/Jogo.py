...
def carregaJogo(Player):
    file = open('jogosalvo.json', 'rb')
    data = file.readline()
    info = json.loads(data.decode())
    Player['Nome'] = info['Nome']
    Player['For'] = info['For']
    Player['Def'] = info['Def']
    Player['HP'] = info['HP']
    Player['SP'] = info['SP']
    Player['Inimigos Mortos'] = info['Inimigos Mortos']
    file.close()
...
def Salvar(Player):
    file = open('jogosalvo.json', 'wb')
    data = dict((k, Player[k]) for k in ('Nome', 'For', 'Def', 'HP', 'SP', 'Inimigos Mortos') if k in Player)
