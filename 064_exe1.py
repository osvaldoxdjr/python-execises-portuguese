...
class SoSimOuNao(Exception):
	def __str__(self):
	    return "Responda somente com Sim ou Nao"
...
class Jogo(object):
    ...
    def apresentação(self):
        mensagem = """
Bem vindo ao jogo das cartas!"
O jogo é bastante simples:
você deve pensar em um número inteiro de 1 a 63 e dizer se o número está
presente no cartões que lhe serão apresentados, serão feitas %i perguntas.
Após responder essas perguntas iremos lhe informar o número que você pensou.
Pronto para o jogo? Que começe o jogo!!!"""%len(self.__cartoes)
        print(mensagem)
 
    def recebeEntradaDoUsuário(self):
	
        while True:
            resposta = input("O número que você pensou está presente no cartão?")	
            try:
                if resposta.upper() == "SIM" or resposta.upper() == "NAO":
    ...
    def imprimeNumeroSecreto(self):
        print("ABRACADABRA! O número que você pensou é: %i"%self.__num)
